...
us_dir  = ('E:\Code\腕手同步识别代码\QT_2threads_36(WiFi)\exe\Mat_file');%~ 超声信号文件路径，在上位机软件中的话，是放在上一级目录下
cd (us_dir)
...
new_part_len = Feature_len/3 - 2 * switch_len;%~ 去掉动作切换数据后的每段数据长度
...
pc1=[pc1hc; pc1wr; pc1com];

%~ pc2阶段数据截取
pc2hc = pc2(1+switch_len: part_len-switch_len, :);
pc2wr = pc2(part_len+1+switch_len: part_len*2-switch_len, :);
pc2com = pc2(part_len*2+1+switch_len: part_len*3-switch_len, :);
pc2=[pc2hc; pc2wr; pc2com];
...
if((mean(pc1(1:new_part_len))-0.5)>0)
    pc1 = pc1inverse;
end
if((mean(pc2(new_part_len+1:new_part_len * 2))-0.5)>0)
    pc2 = pc2inverse;
end
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
%%~ pc1第一阶段数据修正
pc1(1:new_part_len) = 0;
%%~ pc1第二阶段数据修正
temp = pc1(new_part_len+1:new_part_len*2);
temp = mapminmax(temp', 0, 1)';
pc1(new_part_len+1:new_part_len*2) = temp;
...
pc2(new_part_len+1:new_part_len*2) = 0;
%re normalization %~ 第三阶段数据单独再次归一化
temp = pc2(new_part_len*2+1:new_part_len*3);
temp = mapminmax(temp', 0, 1)';
pc2(new_part_len*2+1:new_part_len*3) = temp;

%%~ 标记出第一阶段 PC2数据的峰值和谷值
% fre prior
figure;
pc2hc = pc2(1: new_part_len, :);%~ 第一阶段，hand closing的PC2数据
[pk,lkp] = findpeaks(pc2hc, 'MinPeakHeight',0.4);%~ findpeaks查找局部最大值，返回pk为峰值，lkp是峰值的索引
plot(1:new_part_len,pc2hc,lkp,pk,'o')%~ 将最大值标记出来'o'
hold on;
[pk,lkr] = findpeaks(-pc2hc);%~ lkr是谷值的索引
plot(lkr,-pk,'*') %~ 将最小值标记出来'*'
hold off;
...
%%~ 标记出第三阶段 PC2数据的峰值和谷值
pc2wr = pc2(new_part_len+1: new_part_len*2);%~ 第二阶段wristrotation为0
pc2com = pc2(new_part_len*2+1: new_part_len*3);%~ 第三阶段同步运动数据
figure;
[pk,lkp] = findpeaks(pc2com, 'MinPeakDistance', 6, 'MinPeakHeight',0.4);
plot(1:new_part_len,pc2com,lkp,pk,'o')
hold on;
