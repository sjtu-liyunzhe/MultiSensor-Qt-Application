...
namespace kd
{
    ...
    public:
        ...
        KDTree(std::vector<std::pair<kd::Point<N>, ElemType>>& points);
        ...
        bool contains(const kd::Point<N>& pt) const;
         ...
         */
        void insert(const kd::Point<N>& pt, const ElemType& value = ElemType());
         ...
         */
        ElemType& operator[](const kd::Point<N>& pt);

        /*
         * Returns a reference to the key associated with the point pt. If the point
         * is not in the tree, this function throws an out_of_range exception.
         */
        ElemType& at(const kd::Point<N>& pt);
        const ElemType& at(const kd::Point<N>& pt) const;
         ...
         */
        ElemType kNNValue(const kd::Point<N>& key, std::size_t k) const;
    ...
    private:
        struct Node {
            ...
            ElemType value;
            Node(const kd::Point<N>& _pt, int _level, const ElemType& _value = ElemType()) :
         ...
         */
        Node* buildTree(typename std::vector<std::pair<kd::Point<N>, ElemType>>::iterator start,
            typename std::vector<std::pair<kd::Point<N>, ElemType>>::iterator end, int currLevel);
         ...
         */
        Node* findNode(Node* currNode, const kd::Point<N>& pt) const;

        // Recursive helper method for kNNValue(pt, k)
        void nearestNeighborRecurse(const Node* currNode, const kd::Point<N>& key, BoundedPQueue<ElemType>& pQueue) const;
    ...
    template <std::size_t N, typename ElemType>
    typename KDTree<N, ElemType>::Node* KDTree<N, ElemType>::buildTree(typename std::vector<std::pair<kd::Point<N>, ElemType>>::iterator start,
        typename std::vector<std::pair<kd::Point<N>, ElemType>>::iterator end, int currLevel) {
        if (start >= end) return NULL; // empty tree

        int axis = currLevel % N; // the axis to split on
        auto cmp = [axis](const std::pair<kd::Point<N>, ElemType>& p1, const std::pair<kd::Point<N>, ElemType>& p2) {
    ...
    template <std::size_t N, typename ElemType>
    KDTree<N, ElemType>::KDTree(std::vector<std::pair<kd::Point<N>, ElemType>>& points) {
    ...
    template <std::size_t N, typename ElemType>
    typename KDTree<N, ElemType>::Node* KDTree<N, ElemType>::findNode(typename KDTree<N, ElemType>::Node* currNode, const kd::Point<N>& pt) const {
    ...
    template <std::size_t N, typename ElemType>
    bool KDTree<N, ElemType>::contains(const kd::Point<N>& pt) const {
        auto node = findNode(root_, pt);
        return node != NULL && node->point == pt;
    }

    template <std::size_t N, typename ElemType>
    void KDTree<N, ElemType>::insert(const kd::Point<N>& pt, const ElemType& value) {
    ...
    template <std::size_t N, typename ElemType>
    const ElemType& KDTree<N, ElemType>::at(const kd::Point<N>& pt) const {
        auto node = findNode(root_, pt);
    ...
    template <std::size_t N, typename ElemType>
    ElemType& KDTree<N, ElemType>::at(const kd::Point<N>& pt) {
        const KDTree<N, ElemType>& constThis = *this;
        return const_cast<ElemType&>(constThis.at(pt));
    }

    template <std::size_t N, typename ElemType>
    ElemType& KDTree<N, ElemType>::operator[](const kd::Point<N>& pt) {
        auto node = findNode(root_, pt);
    ...
    template <std::size_t N, typename ElemType>
    void KDTree<N, ElemType>::nearestNeighborRecurse(const typename KDTree<N, ElemType>::Node* currNode, const kd::Point<N>& key, BoundedPQueue<ElemType>& pQueue) const {
        if (currNode == NULL) return;
        const Point<N>& currPoint = currNode->point;
    ...
    template <std::size_t N, typename ElemType>
    ElemType KDTree<N, ElemType>::kNNValue(const kd::Point<N>& key, std::size_t k) const {
